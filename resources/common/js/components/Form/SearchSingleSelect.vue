...
<template>
  <div>
    <listbox :model-value="selectedItem" @update:model-value="(item) => $emit('update:modelValue', item?.id ?? null)">
      ...
      <div class="w-full relative">
        ...
        >
          <span>{{ selectedItem?.name ?? "すべて" }}</span>
        ...
        <transition enter-active-class="animate-fade-in" leave-active-class="animate-fade-out">
          ...
          >
            <listbox-option
              v-if="showAll"
              :value="null"
              :class="modelValue === null ? 'bg-rose-100/70' : ''"
